...
\todo[inline]{Consider changing from FLoating-point OP... to Floating-point Op.. (acr. FLOPS)}
...
\todo[inline]{Maybe mention that MPI requires node 0 (rank == 0) to do some
extra pre-processing? ADDED: Review this:}
...
not benefit much (if at all) by parallelization. Either way, these parts are negilible parts of the overall runtime for problem sizes of any considerable size.  
