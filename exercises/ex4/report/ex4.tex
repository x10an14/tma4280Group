...
\pi^2}{6}$. It is the difference between this sum and the sum of finite length
Hyperharmonic series the problem set is interested in, and our program
calculates both serially and in parallel.
...
neglegible extra memory needed for OpenMP. This is because a set of threads
...
When running on 3 Open MP threads with no MPI, the runtime behaved about cubically
with respect to the input size ($n=2^k$). However, it showed no significant increase in
runtime until k = 17, something which indicates that the initialization and overhead dominates the runtime
in the problem sizes specified in the task, so the size must be increased to gain any significant data.

\todo[inline]{Add more data on more OMP threads if time allows.}
