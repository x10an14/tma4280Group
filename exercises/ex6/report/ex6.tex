...
see from the plot that the bandwith starts to limit the paralellization around
...
increases past the scope we have tested on, one could see sevaral more such
points, where dividing across 9 nodes as we have done at the most in our testing
would also start seeing falloff in paralell efficiency as it would have to send
to much data between each of the nodes.
...
problem. Solving the problem with the DST takes $n^2log(n)$ floating point
