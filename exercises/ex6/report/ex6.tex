...
\begin{enumerate}
	...
	all processes, by creating a matrix $\mathbf{A}$ representing a grid of all unknowns, which fills each process with $\sim \frac{N-1}{P \thickspace (=
	...
	due to the discretization of the 2D Poisson problem, $h^2, \thickspace h =
	\frac{1}{N}$.
...
Poisson operation. Hence, each process $p$,
