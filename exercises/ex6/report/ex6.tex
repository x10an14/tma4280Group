...
\todo{Too many times since in this paragraph}
...
\paragraph{Complexity of Step~\ref{istep-gen}}

This step generates the datastructures with which the solver performs the 2D
Poisson operation. Hence, in step~\ref{istep-gen}, the following datastructures
are created with the following operations inside of each process;

(From here on out, $N$ is the input size given as parameter to the solver).
\begin{itemize}
	...
	is simplified from being $\mathcal{O}(N^2 + \frac{(N-1)^2}{P}$, to $\mathcal
	{O}(N^2\times \frac{1-\frac{2}{N}+\frac{1}{N^2}}{P}$, and finally into
	$\mathcal{O}(N^2)$).

	\item The ``transpose matrix'', called $\mathbf{B}$ in step~\ref{istep-fst1}
	is identical in size and run-time complexity when comparing with matrix
	$\mathbf{A}$; $\mathcal{O}(N^2)$. This one is also initialized in step~\ref
	{istep-gen}.
	...
	function, if you will). It's complexity in run-time will be constant, once
	...
	greater complexity beyond a constant added; $\mathcal{O}(N^2 + C)$. Which
	again can be simplified into $\Rightarrow \mathcal{O}(N^2)$.
\end{itemize}
...
Text~\cite{Insert Reference Here}, namely $f(x,y) = 5\pi^2 sin(\pi x) sin(2\pi 
y)$. 
...
\begin{centering}
	\includegraphics[scale=0.425]{pics/pX.png}
\end{centering}
...
more time than sending less to more nodes.
...
\todo{Should we use an other letter for the matrix A? It might confuse with the 
matrix A above.}
\begin{displaymath}
